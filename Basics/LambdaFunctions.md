...
#### Захват по умолчанию
Существует два способа захвата по умолчанию: по значению (`=`) и по ссылке (`&`).
...
> 
> - Майерс, Эффективный и современный C++ (2016)

- Когда создается объект лямбды (т.е. объект, у которого эта лямда по факту является `operator()`, а захваченные переменные - переменными этого класса), появляется "Замыкание".
